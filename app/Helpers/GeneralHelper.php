...
function format_decimal($number,$prefix=2)
{
    return number_format((float)$number, $prefix, '.', '');
}

function calculate_year_decimal($start,$end)
{
...
function calculate_year_integer($start,$end){
    $birthDate = new \DateTime($start);
	$today = new \DateTime($end);
    $year = $today->diff($birthDate)->y;
